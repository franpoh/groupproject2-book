...
class accessController {
    ...
    async login(req, res) {
        ...
        if (result.status == 200) {
            cookie('refreshToken', result.data.refreshToken, { httpOnly: true, sameSite: "None", secure: true });
            cookie('accessToken', result.data.accessToken, { httpOnly: true, sameSite: "None", secure: true });
            return res.status(result.status).json({ message: result.message });
