...
class accessController {
    ...
    async login(req, res) {
        ...
        if (result.status == 200) {
            res.cookie('refreshToken', result.data.refreshToken, { path: "/", domain: "book-libraryshop.herokuapp.com", httpOnly: true, sameSite: "None", secure: true });
            res.cookie('accessToken', result.data.accessToken, { path: "/", domain: "book-libraryshop.herokuapp.com", httpOnly: true, sameSite: "None", secure: true });
            return res.status(result.status).json({ message: result.message });
